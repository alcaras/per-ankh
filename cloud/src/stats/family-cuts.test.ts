...
import { buildFamilyCutTable, type FamilyCutInput } from "./family-cuts";
...
function game(nation: string, fielded: readonly string[]): FamilyCutInput {
	return { nation, family_classes: fielded };
}

// Rome's pool, minus one — the shape of every four-family player-game.
function romeCutting(family: string): FamilyCutInput {
	return game(
		"NATION_ROME",
...
describe("buildFamilyCutTable", () => {
	it("reads indifference as a baseline, not as a preference", () => {
		...
		const table = buildFamilyCutTable(ROME.map(romeCutting));

		expect(table.player_games).toBe(4);
		for (const row of table.rows) {
			expect(row.cut_pct).toBeCloseTo(25, 6);
			expect(row.baseline_pct).toBeCloseTo(25, 6);
	...
	it("accumulates the baseline per game when pools differ in size", () => {
		...
		const table = buildFamilyCutTable([
	...
	it("counts eligibility by pool membership, not by nation", () => {
		...
		expect(champions.cut).toBe(1);
	});

	it("finds a family that is genuinely refused", () => {
		// Patrons cut in every one of forty Rome games.
		const table = buildFamilyCutTable(
		...
		expect(patrons.cut_pct).toBe(100);
		expect(patrons.baseline_pct).toBeCloseTo(25, 6);
		expect(patrons.z).toBeGreaterThan(5);
		expect(patrons.significant).toBe(true);
		// And it leads the table, which is sorted most-refused first.
		expect(table.rows[0].family_class).toBe("FAMILYCLASS_PATRONS");
	});
	...
	it("excludes a nation that fields its whole pool", () => {
		...
		const table = buildFamilyCutTable([game("NATION_TAMIL", tamil)]);
