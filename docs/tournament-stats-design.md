...
> **Update (2026-08-30) — three points superseded by the global-stats build** (`docs/global-stats-design.md`, `docs/aggregate-statistics.md`). The rest of the doc still stands; these three are stated as current fact and are no longer true.
>
> 1. **§5 and §7 stage-3 item 10: `ChartSpec`/`StatsView` are no longer typed against `ChartBundle`.** They take `ChartBundleCore`, done for `/stats` — so "reusing the shell means parameterizing those types" is a cost already paid, and item 10's "do not parameterize `ChartSpec`/`StatsView` yet" is spent advice. It needed no generic parameter and no discriminant: `ChartBundle` extends `ChartBundleCore`, so it satisfies the predicates structurally. What §5 says about the *shell's* limits is unchanged — `buildOption` is still a hardcoded switch on spec id, and yields/families/laws/tech are still anchor-only stubs special-cased in the template dispatch. The tabs have **not** collapsed onto the registry; that refactor stays deferred.
> 2. **§7 stage-3 item 5 and §6: `save_dates`/`favorite_day_of_week` no longer "stay in the core."** At `BUNDLE_SCHEMA_VERSION` 9, `save_dates` moved to the user-only `ChartBundle` and `favorite_day_of_week` was deleted outright. The open question these sections defer — whether the tournament page renders a save-date calendar — is answered by the fact that it never did, and the global corpus made the field's `O(games)` size the deciding argument.
> 3. **§7 stage-3 item 10's colour guidance points at the wrong helper now.** Categorical series take `getSeriesColor(i)`; `getChartColor(i)` is the civilization fallback ramp. See `docs/reference/color-scheme.md`.
