...
| `slug_claim_attempt` | 15 / hr per user | `POST /v1/users/me/slug` (counts attempts, not successes) |
...
- **Errors:** `401 UNAUTHORIZED`, `400 INVALID_SLUG` (bad format **or** reserved; the message states the rule and is safe to show verbatim), `409 SLUG_TAKEN` (another user holds it), `409 SLUG_ALREADY_SET` (the caller already claimed one), `429 RATE_LIMIT_SLUG_CLAIM` (15/hr per user), `415 UNSUPPORTED_MEDIA_TYPE`, `400 INVALID_BODY`.
- **Notes:** Uniqueness is enforced by the `users.slug` unique index rather than a pre-check, so two simultaneous claims of the same name can't both win. The conditional `UPDATE … WHERE slug IS NULL` bounds how often the claim can *succeed* (once per account) but not how often it can be called — an unclaimed user's rejected attempts are real D1 writes and a name-availability probe — so the budget counts **attempts**, in its own `slug_claim_attempt` bucket (24h retention, metadata-free) rather than off the one-per-account `slug_claim` row. Set-once is a support-burden decision, not a link-integrity one: `/users/<user_id>` stays a permanent permalink either way. No API path releases or renames a claimed slug — clearing one is an operator action against the row. A successful claim writes a `slug_claim` audit event (awaited; 90-day retention).
