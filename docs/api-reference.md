...
The five bundle fields the home page's stats panels draw, over the unfaceted `duel` slice — a trimmed, public projection of the entry [`GET /v1/stats`](#get-v1stats) serves signed-in visitors by default.
...
- **Response 200:** `{ summary: HomeStatsSummary | null }` — `summary` carries `nationWinRate`, `expansionWinRate`, `capitalFamilyWinRate`, `startingArchetypeWinRate`, `techFirst`, and `meta.game_count`. An envelope rather than six nullable fields: one KV entry answers for all of them, so they are present together or not at all. `null` on a cache miss, which the home page renders by dropping the whole stats region rather than showing empty charts.
- **Errors:** `429 RATE_LIMIT_HOME_SUMMARY`.
- **Notes:** Trimmed because the full bundle's bulk is `wonderStats` / `yieldCurves` / `lawTiming`, none of which home reads, and home is the page most likely to be a visitor's first byte of the site. Each field ships whole except `startingArchetypeWinRate`, which is floored at **50 games** server-side (`HOME_ARCHETYPE_MIN_GAMES`) — a correctness filter, not a presentation one: without it the headline reads "Diplomat 73%" off eleven games. So home's archetype panel and `/stats`' show different rows for the same corpus, deliberately and without caveat text; `/stats` is the surface for the whole distribution. The row caps the panels apply (top 7, ranked by games played) are the frontend's, so they move without a Worker deploy. Never computes on a miss — what keeps misses rare is the nightly precompute plus the hourly warm (`STATS_WARM_CRON`), which cover exactly the unfaceted slices this reads. Its own `home_summary_view` bucket (600 reads/hr per IP), one slot per home page load. Edge-cached 60s (`s-maxage`), no browser cache.
