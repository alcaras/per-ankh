...
- **Auth:** Session.
- **Response 200:** `{ opponents: [{ user_id, display_name, slug, avatar_url, meetings, badges }], computed_at, rated }`. `meetings` is how many rated games the pair has already played; `badges` is a subset of `active_this_week` / `new_here` / `bridges_circles`; `computed_at` is when the list was last rebuilt (`null` on an empty list); `rated` is whether the viewer has any rated multiplayer game at all, which is what separates "nothing yet" from "nothing this week".
- **Errors:** `401 UNAUTHORIZED`.
