...
- **Auth:** Public.
- **Query:** `since` / `until` (optional, `YYYY-MM-DD`, `until` exclusive) — counts only games uploaded in the window (`created_at`, server-authoritative); the `/players` page's season boards, past seasons included. Invalid values are `400 INVALID_QUERY`. A closed window (`until` in the past) is immutable and caches for a day.
...
- **Notes:** Its own `season_view` bucket (1200 reads/hr per IP; scraper UAs exempt), not a share of `anon_read`. Exposes only display names and counts — never online ids. Cached `public, max-age=300, s-maxage=60`. Backfill for pre-0044 rows is the admin reindex sweep (no re-uploads).
