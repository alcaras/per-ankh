...
| `season_view` | 1200 reads / hr per IP | `GET /v1/stats/players`. Its own budget rather than a share of `anon_read`: `/season` and `/games/*` are different populations, and pooling them would let a crawl of game pages decide when the season board starts refusing. Not a share of `global_stats_view` either — `/season` is anonymous where `/stats` is session-gated. Two reads per Season page load (all-time + the selected season), so 1200 is ~600 page loads an hour. Own ceiling var (`SEASON_VIEW_PER_HOUR`), retuned like the others |
