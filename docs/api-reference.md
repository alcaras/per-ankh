...
| `season_view` | 1200 reads / hr per IP | `GET /v1/stats/players`. Its own budget rather than a share of `anon_read`: `/players` and `/games/*` are different populations, and pooling them would let a crawl of game pages decide when the players board starts refusing. Not a share of `global_stats_view` either — `/players` is anonymous where `/stats` is session-gated. Two reads per `/players` load (all-time + the selected season), so 1200 is ~600 page loads an hour. Own ceiling var (`SEASON_VIEW_PER_HOUR`), retuned like the others |
...
- **Auth:** Public.
- **Query:** `since` / `until` (optional, `YYYY-MM-DD`, `until` exclusive) — counts only games uploaded in the window (`created_at`, server-authoritative); the `/players` page's season boards, past seasons included. Invalid values are `400 INVALID_QUERY`. A closed window (`until` in the past) is immutable and caches for a day. The `/players` page also sends `v` — its row-shape version (`PLAYERS_SHAPE_VERSION`, `src/lib/api-cloud.ts`), unread by the Worker: the URL is the cache key, so bumping it orphans the day-cached archive boards that predate a new field instead of leaving them a day behind the deploy.
- **Response 200:** `{ players: [{ user_id, display_name, avatar_url, duels_network, duels_cloud, ffas, total }] }`, ordered by total desc. `avatar_url` is the player's Discord avatar, built server-side from the stored hash (`buildAvatarUrl`, Discord's default avatar when they never set one) — the same select-use-don't-serialize shape the profile and standings payloads use. Playing is counted, not uploading: anyone's upload credits every human seat — the uploader via their claimed seat, others by matching the seat's online id (`player_summaries.online_id`, migration 0044) against `user_online_ids`. Matches double-uploaded by both players count once per player (deduped on `xml_game_id`). **Public games only** (`is_public = 1`, on every credit path) — the same visibility rule the profile card and the global corpus enforce: a save its owner kept private counts for nobody, and a match counts as soon as any one of its uploads is public. Duel = exactly two humans, split by game mode (`NETWORK` / `PLAY_BY_CLOUD`); FFA = 3+ humans; the remainder is derived client-side.
- **Errors:** `429 RATE_LIMIT_SEASON`.
- **Notes:** Its own `season_view` bucket (1200 reads/hr per IP; scraper UAs exempt), not a share of `anon_read`. Exposes only display names, Discord avatars, and counts — never online ids. Cached `public, max-age=300, s-maxage=60`. Backfill for pre-0044 rows is the admin reindex sweep (no re-uploads).
