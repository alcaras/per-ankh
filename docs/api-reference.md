...
- **Errors:** `429 RATE_LIMIT`.
- **Notes:** `anon_read` bucket (200/hr per IP; scraper UAs exempt). Exposes only display names and counts — never online ids. Cached `public, max-age=300, s-maxage=60`. Backfill for pre-0044 rows is the admin reindex sweep (no re-uploads).
