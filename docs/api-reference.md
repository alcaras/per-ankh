...
- **Response 200:** `{ summary: HomeStatsSummary | null }` — `summary` carries `nationWinRate`, `capitalFamilyWinRate`, `startingArchetypeWinRate`, and `meta.game_count`. An envelope rather than a handful of nullable fields: one KV entry answers for all of them, so they are present together or not at all. `null` on a cache miss, which the home page renders by dropping the whole stats region rather than showing empty charts.
