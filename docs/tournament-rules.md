...
- **Late pairing** (`handleAddRoundMatch`, `POST …/rounds/:round_id/matches`): an admin can add a match between two **active, unpaired** same-division slots to the still-**open** Swiss round — the catch-up game for a substitute reinstated after their division's current round was paired. The map is auto-assigned by the same engine as round generation, the result feeds standings and the next round's pairing like any other, and the round won't auto-close until the added match is reported. Swiss only; the championship bracket is never hand-edited. The Swiss standings table surfaces this as a **Pair** action on eligible rows (admin view).
...
- **Caster self-service:** any logged-in user can add/move/remove **themselves** on a pending match's part (`…/parts/:part_id/casters/me`, `player.ts`) — streamer if the part has none, else co-caster; a self "streamer" move bumps the current streamer to co-caster. **Casting is always third-party:** a match's own two players can't cast it (**403 `PARTICIPANT_CANNOT_CAST`**), and the inline cast buttons don't render on your own match — though it still shows the "needs a caster" flag, because it does. Casting is pending-only, but **removing yourself** also works on decided matches, and on a match you're playing in (you shouldn't stay credited for a cast that never happened, and nobody credited before the third-party rule gets stuck). The Matches page's **Cast** view lists upcoming parts still needing a caster; the Matches filters are URL-addressable (e.g. `?view=cast`, `?status=scheduled`).
