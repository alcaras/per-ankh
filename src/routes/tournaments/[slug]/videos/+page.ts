...
export const load: PageLoad = async ({ parent, fetch }) => {
	...
	const archive = await cloudApi
		.getTournamentVideoArchive(tournament.tournament_id, { fetch })
		.catch((err: unknown) => {
			// A spent read budget is the one failure worth showing: an empty grid
			// is indistinguishable from a tournament with no playlist, so the
			// visitor would be told there are no videos rather than to come back.
