...
<main class="cloud-scroll flex-1 overflow-y-auto px-4 py-8" use:autohideScroll>
	<div class="mx-auto max-w-xl space-y-4">
		...
		>
			...
			>
				...
				{:else if section === "tournament"}
					...
					/>
					<p class="text-xs text-tan">
						The saves linked to the tournament's completed matches — the same
						set its stats page aggregates.
					</p>
				{:else if section === "date"}
					<div class="flex items-center gap-2">
						<label class="flex-1 text-xs text-tan">
							...
							<input
								...
								onchange={(e) => selectDate("from", e.currentTarget.value)}
					...
					<p class="text-xs text-tan">
						Upload date (UTC), both bounds inclusive. Either may be left empty
		...
		>
			...
			>
				<p class="mb-3 text-xs text-tan">
					Reparse each game in the section whose stored parser version is older
					than the current build. Downloads, parses, and re-uploads each game in
					this browser tab. Audited as <code class="text-orange"
						>admin_reimport</code
					>.
				</p>
		...
		>
			...
			>
				<p class="mb-3 text-xs text-tan">
					Rebuild the section's derived D1 tables (summaries, per-turn series,
					tech/law/wonder events, wonder pools) from each stored blob. No
					...
					upload (e.g. per-turn victory points). Audited as <code
						class="text-orange">admin_reindex</code
					>.
				</p>
				<div class="mb-3 text-sm text-tan">
					{#if data.allGames.length === 0}
						{isFiltered ? "No games in this section." : "No games to reindex."}
					{:else}
