<script lang="ts">
	...
	import { formatScheduledWithLocal } from "$lib/utils/formatting";

	let { matches, slots }: { matches: TournamentMatch[]; slots: SlotMaps } =
		$props();

	// Upcoming sittings previewed here. Live ones are never capped — a match
	// being played right now is the most time-sensitive thing this panel has.
	...
	// UTC first with the viewer's own clock in parentheses — an absolute time,
	// not a relative one, because "in 1 day" is what a reader has to translate
	// and the exact instant is what they came for.
		...
		{#each rows as { np, live } (np.match.match_id + ":" + np.partNumber)}
			<li class="text-xs">
				<div class="flex items-center gap-1.5">
					{#if live}
						<span
							class="shrink-0 rounded bg-orange px-1 text-[10px] font-bold text-black"
							>LIVE</span
						>
					{/if}
					<PlayerAvatar
...
{:else}
	<p class="text-xs text-tan opacity-70">No matches scheduled right now.</p>
{/if}
